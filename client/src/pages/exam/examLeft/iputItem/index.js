...
class InputItem extends Component {
  ...
  showText = (itemKey) => {
    let convertAnswer = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'];
      ...
      return (
        <div className='text-area-block'>
          <TextArea className='text-area-add' name={`item${itemKey}textArea${k}`}
            ...
            placeholder={`Nhập đáp án ${convertAnswer[k]}`}
    ...
    if (checkNullValues === 0) {
      return (
        <div className='text-input-block'>
          <Input className='input-add-text-area'
            placeholder={`Nhập vào câu hỏi câu ${itemKey + 1}...`}
      ...
      return (
        <>
          ...
          <div className='text-input-block'>
            <Radio.Group onChange={(e) => this.isChange(e)}
              onChange={(e) => this.isChange(e)}
              name={`item${itemKey}radio`} onChange={(e) => this.isChange(e)} >
